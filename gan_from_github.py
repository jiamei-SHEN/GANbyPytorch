...
class Discriminator(nn.Module):
    ...
    def forward(self, x):
        x = F.leaky_relu(self.fc1(x), 0.2)
        x = F.dropout(x, 0.3)  # 防止过拟合的一种正则化技术
        x = F.leaky_relu(self.fc2(x), 0.2)
        x = F.dropout(x, 0.3)
        x = F.leaky_relu(self.fc3(x), 0.2)
        x = F.dropout(x, 0.3)
...
torch.save(G.state_dict(), f'generator_bs_{bs}_epoch_{n_epoch}.pth')
torch.save(D.state_dict(), f'discriminator_bs_{bs}_epoch_{n_epoch}.pth')
